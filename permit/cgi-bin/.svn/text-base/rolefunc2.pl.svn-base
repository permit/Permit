...
sub get_functions {
  ...
  my $stmt = "select f.function_id, f.function_name, f.function_description,
	       ...
	       CASE IFNULL(f.primary_authorizable,'N')  
			WHEN 'Y' THEN 'Yes' 
			WHEN 'N' THEN 'no' 
			WHEN 'D' THEN 'DLC-based' 
			ELSE 'no' END,
               -- decode(IFNULL(f.primary_authorizable,'N'),
...
--                 'Y', 'Yes', 'D', 'DLC-based', 'N', 'no', 'no'),
  ...
  #print "'$stmt2'<BR>";
  my $csr2 = $lda->prepare("$stmt") or die( $DBI::errstr . "\n");
  $csr2->execute();
...
sub get_dept_based_function {
  ...
  #
  my ($function_name, $dept_code, $dept_name);
  while ( ($function_name, $dept_code, $dept_name) = $csr->fetchrow_array() ) {
