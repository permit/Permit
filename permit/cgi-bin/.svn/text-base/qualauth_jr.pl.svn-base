...
#       06/15/05 (Add support for sensitive qualtypes and implied auths)
#
...
#
 $all_levels = 20;
 $g_auth_table = 'authorization2';  # Look at union of real and implied auths
...
$g_flag_inactive_qual = 1;  # Set the switch to find and flag inactive quals
...
#
#  Find out if the qualifier_type is "sensitive", i.e., if you need
#  special authority to view the qualifier names.
#
 $g_qualtype_is_sensitive = &is_qualtype_sensitive($lda, $qt);
 #print "Is qualifier_type '$qt' sensitive? '$g_qualtype_is_sensitive'<br>";

#
# Make another adjustment to the URL stem for this CGI script.
# If no cat. specified and qualtype is not sensitive, change https -> http
#
 if (!($cat) && ($g_qualtype_is_sensitive ne 'Y')) { 
   $url_stem =~ s/^https/http/;
 }

#
#  If we're going to display authorizations in a category, 
#  or if we are going to display qualifiers within a "sensitive"
#  qualifier_type, then parse certificate information.  Otherwise, skip it.
#
# Is there a category or is this a sensitive qualtype?
if (($cat) || ($g_qualtype_is_sensitive eq 'Y')) {  
...
#
#  Find a list of qualifier_id's for qualifiers that are flagged as
#  "inactive".
#
 %inactive_qualid = ();
 if ($g_flag_inactive_qual) {
   &get_inactive_qualifiers($lda, $qt, $rootnode, \%inactive_qualid);
 }
...
#
# Find out if the user is authorized to view a given qualifier_type
#
 $g_can_view_qualtype = 'N';
 if ($g_qualtype_is_sensitive eq 'Y') {
   $g_can_view_qualtype = &check_auth_for_qualtype($lda, $qt, $k_principal);
 }
 #print "Auth to view qualtype '$qt' is '$g_can_view_qualtype'<BR>";

#
# If this is a "sensitive" qualifier_type and if 
# the user is authorized to see qualifier name for this qualifier_type
# then set table_name to QUALIFIER2;
# otherwise, set the table_name to QUALIFIER.
# >>>>> Eliminate the view PROXY_QUALIFIER2 -- it is not practical.
#
 if ($g_qualtype_is_sensitive && $g_can_view_qualtype eq 'Y') {
     $g_qual_table = 'QUALIFIER2';
 }
 else {
     $g_qual_table = 'QUALIFIER';
 }
 #print "Table name is '$g_qual_table'<BR>";
...
#
 ...
 $stmt1 = "select q1.qualifier_id, q1.qualifier_code," 
          ...
          . " q2.qualifier_code"
          . " from $g_qual_table q1, primary_auth_descendent pad, qualifier q2"
 ...
 {
    print "Error preparing statement 1 (finding qualid, code, name).<BR>";
    die;
 }
...
#
 ...
 $stmt2 = "select q1.qualifier_id, q1.qualifier_code,"
          ...
          . " $g_qual_table q1, primary_auth_descendent pad, qualifier q2"
          . $hide_term3_sql_frag1
          . " where qc.parent_id = ?"
 ...
 ## Added 9/24/2004: Use RULE based optimizer ##
 $stmt2 =~ s/select/select \/\*\+ RULE \*\//g;
 #print $stmt2 . "<BR>";
...
#
 ...
 if ( ((!$is_big_branch) && (!$suppress) && (!$child_code) ) 
      || (($no_leaf eq '1') && ($qt eq 'COST') && (!$child_code) ) ) {
...
#
 ...
 for ($i = 0; $i < $n; $i++) {
   ...
   $qname_string = ($itemtype[$i] eq 'Q')
                   ...
                   : $qname[$i];
   # Make the qualifier name red if qualifier is inactive
   if ($inactive_qualid{$qid[$i]}) {
       $qname_string = "<font color=red>$qname_string</font>";
   }
...
sub get_qualid_list {
  ...
  $stmt6 = "select distinct a.qualifier_id"
     . " from $g_auth_table a, qualifier_descendent qd"
     . " where"
...
#
#  Subroutine 
#   get_inactive_qualifiers($lda, $qualtype, $rootnode, \%inactive_qualid)
#  
#  Build a hash %inactive_qualid setting $inactive_qualid{$qualid} = 1
#  for each qualifier id in the requested branch of the tree whose
#  status is 'I'.
#
###########################################################################
sub get_inactive_qualifiers {
  my ($lda, $qualtype, $rootnode, $rinactive_qualid) = @_;
  my ($csr, $stmt11, $inactive_qual_id);
  $stmt11 = "select distinct q.qualifier_id"
  ...
  unless ($csr = $lda->prepare($stmt11)) 
  {
     print "Error preparing statement 6.<BR>";
     die;
  }
  $csr->execute;
  while ( ($inactive_qual_id) = $csr->fetchrow_array ) {
     $$rinactive_qualid{$inactive_qual_id} = 1;
  }
  $csr->finish;
}
...
sub get_child_id {
  ...
  $csr->finish;
  if (!($child_id)) {
    print "Error: Qualifier '$child_code' of type '$qualtype' not found<BR>";
    die;
  }
  return ($child_id);
}
...
sub get_special_select {
  ...
  my $stmt = 
      "select q1.qualifier_id, q1.qualifier_code,"
       ...
       . " q1.qualifier_level"
       . $sql_dept_frag1
       . " from qualifier_child qc0, $g_qual_table q1, qualifier_child qc,"
       . " qualifier q2"
       . $sql_dept_frag2
       . " where qc0.parent_id = ?"
