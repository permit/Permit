...
sub get_select_stmt {
    ...
    if ($sort_opt eq '1') {
      ...
      $sql_frag4 = "p.last_name, p.first_name, a.function_name, a.qualifier_code, q3.qualifier_code";
    }
    if ($filter_opt eq '1') {  # Show only auths. with qualcodes for the DLC
      return ("select a.kerberos_name, a.function_name,"
             ...
             . " q3.qualifier_code"
             . " from authorization a left outer join primary_auth_descendent pa"
	     . " on  pa.child_id = a.qualifier_id and pa.is_dlc = 'Y'" 
             . " left outer join qualifier q3 on q3.qualifier_id = pa.parent_id"
	     . " , person p left outer join qualifier q2"
	     . " on  q2.qualifier_code = p.dept_code and q2.qualifier_type = 'ORGU'"
             . " , qualifier q"
             ...
             . "   and a2.function_category = '$g_category'"
             . "  union"
             . "  select distinct a2.authorization_id"
             . "   from authorization a2"
             . "   where a2.qualifier_code = '$picked_qual'"
             . "   and a2.function_category = '$g_category')"
             . " and a.function_category = '$g_category'"
      ...
      return ("select a.kerberos_name, a.function_name,"
             ...
             . " q.qualifier_name, $sql_frag1,"
             . " q3.qualifier_code"
             . " from authorization a left outer join primary_auth_descendent pa on pa.child_id = a.qualifier_id and pa.is_dlc = 'Y' "
             . " left outer join qualifier q3 on q3.qualifier_id = pa.parent_id, qualifier q"
