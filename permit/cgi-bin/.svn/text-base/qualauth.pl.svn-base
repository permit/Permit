...
#       08/29/05 (Properly identify 8-digit HR Org. Units in DEPT hierarchy)
#       12/01/05 (Disallow viewing all levels of the PCCS hierarchy by
#                 adding high-level nodes to @big_branch array)
#       05/22/06 (Add '*' link to cost objects in 'PCCS' hierarchy as well.)
#       05/25/06 (Only check "big branches" for specific hierarchies.)
#       12/04/06 (Handle Function Names containing a dollar sign $)
#       11/28/07 (By default, hide terminated cost obj. for PCCS hierarchy)
#
...
#
 ...
 $url_stem3 = "https://$host/cgi-bin/rolecc_info.pl?";
 $url_stem4 = "https://$host/cgi-bin/rolequal1.pl?";
 $main_url = "https://$host";
...
#
 if (!($cat)) {  # If no category is specified, change https -> http
   # $url_stem =~ s/^https/http/;  ### Leave it as https
 }
...
#
 ...
 if ($rootnode eq '') {
   ...
   die "Invalid or mismatched qualifier type ($qt) or code ($rootqcode)." . "<BR>";
 }

#
#  Figure out if the rootnode specified is a "big branch" of the qualifier
#  tree.  Only check in the "big" hierarchies.
#
 $is_big_branch = 0;  # Default -- not a big branch
 if ($qt eq 'COST' || $qt eq 'FUND' || $qt eq 'PCCS') { # Only for big sets
...
#
 ...
 #$stmt1 = "select /*+ ORDERED */ q1.qualifier_id, q1.qualifier_code," 
# $stmt1 = "select /*+ RULE */ q1.qualifier_id, q1.qualifier_code," 
...
#          . " order by q1.qualifier_code, q2.qualifier_code";
 $stmt1 = "select /*+ RULE */ q1.qualifier_id, q1.qualifier_code," 
          . " q1.qualifier_name, q1.has_child, q1.qualifier_level,"
          . " q2.qualifier_code"
          . " from qualifier q1 left outer join primary_auth_descendent pad on pad.child_id=q1.qualifier_id and pad.is_dlc='Y'"
	  . " left outer join qualifier q2 on q2.qualifier_id=pad.parent_id"
          . " where q1.qualifier_id = ?"
          . " order by q1.qualifier_code, q2.qualifier_code";
 #print $stmt1 . "<BR>";
...
#
 ...
 if (($qt eq 'FUND' || $qt eq 'COST' || $qt eq 'PCCS') 
      && ($hide_term3 eq 'Y')) 
 {
$stmt2 = "select q1.qualifier_id, q1.qualifier_code,"
          ...
          . " qualifier q1 left outer join primary_auth_descendent pad on  pad.child_id = q1.qualifier_id and pad.is_dlc='Y'"
	  . " left outer join qualifier q2 on q2.qualifier_id = pad.parent_id left outer join wh_cost_collector w on "
	  . " w.cost_collector_id = substr(q1.qualifier_code, 2)"
          . " where qc.parent_id = ?"
          . " and q1.qualifier_id = qc.child_id"
    	  . " and IFNULL(w.term_code, '0') <> '3'"
          . $sql_fragment
          . " order by $sort_order";
...
#
 ...
 if ($cat) {  # We only need this SELECT if viewing auths. in a category
   if ($display_detail == 1) {
     $sql_fragment3 = " CONCAT(a.kerberos_name ,' (' , function_name , ')' "
                      . ", ' grant=' , CASE grant_and_view WHEN 'GD' THEN 'Y' ELSE 'N' END" 
                      . " , ' mod. ' , modified_by"
                      . " , ', ' , DATE_FORMAT(modified_date,"
                      . "'%m/%d/%Y'))";
     $sql_fragment3a = ' where';
   }
   elsif ($display_detail == 2) {
     $sql_fragment3 = " CONCAT(a.kerberos_name , ' ' , initcap(p.first_name) , ' '"
                      . " , initcap(p.last_name)"
                      . " , ' (' , function_name , ')')";
     ...
     $sql_fragment3 = " CONCAT(a.kerberos_name , ' ' , initcap(p.first_name) , ' '"
        . " , initcap(p.last_name) , ' - '"
        . " , CASE p.primary_person_type WHEN 'E' THEN 'employee' WHEN 'S' THEN 'student' ELSE 'other' END "
        . " , ' ' , p.dept_code"
        . " , ' (' , function_name , ')'";
     $sql_fragment3a = ", person p"
                       . "  where p.kerberos_name = a.kerberos_name and";
   ...
   if ($category =~ /,/) {  # Multiple categories?
     ...
     $sql_fragment5 = "function_category = '$cat4'";
   }
...
#
 if ($qt eq 'COST' || $qt eq 'FUND' || $qt eq 'PCCS') {
...
#???                ? $bold . "<small>" . $qname[$i] . "</small>" . $endbold
   ...
   #  elsif ($l_code =~ /^PCMIT-/) {
   #    print ' <A HREF="' . $url_stem 
   #         . 'qualtype=PMIT+%28PCMIT-0 nodes and PCs%29'
   #         . '&rootcode=' . $l_code . '&levels=2">'
   #         . "*</A>";
   #  }
...
sub subtype_desc {
  ...
  for ($qualcode) {
    if (/^[0-9]{6}$/) {return $pfx . 'Old Org. unit' . $postfx;}
    elsif (/^PCMIT-0/) {return $pfx . 'PCMIT-0 node' . $postfx;}
    ...
    elsif (/^1[0-9]{7}$/) {return $pfx . 'HR Org. unit' . $postfx;}
  ...
  if ($showdeptname eq 'Y') {
    ...
    $stmt =
      "select q1.qualifier_id, q1.qualifier_code,"
       . " q1.qualifier_name, CASE count(q2.qualifier_id) WHEN 0 THEN 'N' ELSE 'Y' END as YesNo,"
       . " q1.qualifier_level,"
       . " q3.qualifier_code "
       . " from qualifier_child qc0, qualifier q1 left outer join primary_auth_descendent pad on pad.child_id = q1.qualifier_id and pad.is_dlc = 'Y'"
       . " left outer join qualifier q3 on q3.qualifier_id = pad.parent_id"
       . " ,qualifier_child qc left outer join qualifier q2 on q2.qualifier_id = qc.child_id "
       . $sql_frag
       . " where qc0.parent_id = ?"
   ...
   $stmt =
      "select /*+ RULE */ q1.qualifier_id, q1.qualifier_code,"
       . " q1.qualifier_name, CASE count(q2.qualifier_id) WHEN 0 THEN 'N' ELSE 'Y' END as YesNo,"
       . " q1.qualifier_level"
       . " from qualifier_child qc0, qualifier q1, qualifier_child qc"
       . " left outer join qualifier q2 on q2.qualifier_id = qc.child_id "
       . $sql_frag
       . " where qc0.parent_id = ?"
       . " and q1.qualifier_id = qc0.child_id"
       . " and q1.has_child = 'Y'"
