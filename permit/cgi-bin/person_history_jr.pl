...
print "<head><title>perMIT DB Person Changes</title>";
...
<td class="highlightBlock">yellow.</td></tr>
</table>
<p>
ENDOFTEXT
 ...
 #
  ...
  if ($picked_user) {
    ...
    $where = "where ph.kerberos_name in "
          ...
          . "  where nvl(ph2.end_date, to_date('07292002', 'MMDDYYYY')) > "
          . "  greatest(trunc(sysdate-$days),to_date('07292002','MMDDYYYY'))"
          . " ) ";
  ...
  @stmt = ("select ph.kerberos_name, ph.mit_id, "
          . " rtrim(initcap(ph.last_name || ', ' || ph.first_name "
          . "       || ' ' || ph.middle_name)),"
          . " ph.unit_code, ph.unit_name, ph.person_type,"
          . " to_char(ph.begin_date, 'mm/dd/yy'),"
          . " to_char(ph.end_date, 'mm/dd/yy'),"
...
#                      lists match
#    'bgcolor="gray"'  if they don't match
#
###########################################################################
sub set_diff_color {
  ...
  for ($i = 0; $i < $n; $i++) {
    if ($new[$i] eq $old[$i]) {
      ...
      push(@diff_array, "class=\"highlightBlock\"");
    }
  }
