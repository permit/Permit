...
sub get_nondeletable_users {
    my ($lda) = @_;
    my $ndu = {};
    my $ndu_s = $lda->prepare( 
     "select distinct username from special_username");
    #my $ndu_s = ora_open($lda, 
    # "select owner from dba_objects 
    #  union select username from special_username");
    $csr->execute();
...
sub can_add_drop_users {
    ...
    my $auth_s = $lda->prepare($qry);
    $auth_s->bind_param(1, $user);
    my ($canAddDrop) = $auth_s->fetchrow_array();
    $auth_s->finish();
