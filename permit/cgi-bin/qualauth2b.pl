...
#       09/27/02 (Hide DEPT leaves by default when no category is specified)
#       10/29/02 (Option for hiding Qualifier Codes)
#       11/07/02 (Option for expanding authorizations)
#       08/04/03 (Handle 8-digit Org Units)
#
...
if ($group_option eq '') {$group_option = 2;}  # Default is 2
$hide_qc_default = $formval{'hide_qc_default'};
$hide_qc_default =~ tr/a-z/A-Z/;
$hide_qualcode = $formval{'hide_qualcode'};
$hide_qualcode =~ tr/a-z/A-Z/;
$hide_qualcode = ($hide_qualcode) ? $hide_qualcode
                                  : ($hide_qc_default) ? $hide_qc_default
                                  : 'N';
$expand_auth = $formval{'expand_auth'};
$expand_auth =~ tr/a-z/A-Z/;
if ($expand_auth eq '') {$expand_auth = 'N';}
...
if ($cat_qt) {
  ...
  #print "expand_auth='$expand_auth'<BR>";
}
...
#
 ...
 if ($hide_qualcode) {
   $url_stem .= "hide_qualcode=$hide_qualcode&";
 }
 if ($expand_auth) {
   $url_stem .= "expand_auth=$expand_auth&";
 }
...
#
 $epoch = time();
 #$k_principal = 'SMITHMT';  # For testing
...
#
 $funcfont = '<font color=green><i><b>';  # Bold?
...
#
# Make $no_leaf = 1 the default if the qualifier_type is 'DEPT'.
#
 if ( ($no_leaf eq 'D') && ($qt eq 'DEPT') ) {
   $no_leaf = 1;
 }
...
#
 ...
 $sort_order = ($sort_option eq 'NAME' || $hide_qualcode eq 'Y')
...
#
 ...
 if ($cat) {  # We only need this SELECT if viewing auths. in a category
   ...
   my $table_or_view = ($expand_auth eq 'Y') ? 'expanded_auth2' 
                                             : 'authorization';
   my $virtreal_column = ($expand_auth eq 'Y') ? 'virtual_or_real'
                                               : "'R'";
   $stmt3 = "select "
          . "$sql_fragment3 ,"
          . " do_function,"
          . " AUTH_SF_IS_AUTH_CURRENT(effective_date, expiration_date),"
          . " a.function_name, $virtreal_column"
          . " from $table_or_view a"
          . $sql_fragment3a
          . " qualifier_id = ?"
          . " and $sql_fragment5"
	  . " $sql_fragment4";
   $stmt3 .= " order by $sort_order";
   #print $stmt3 . "<br>";
...
@virtual_real = ();  # 'R' for real authorizations, 'V' for expanded
...
#
 # Describe the meaning of green if there are functions displayed in the
 # new format.
 $header_fragment0 = ($group_option eq '2')  # If there are function headers
   ? "<li>" . $funcfont
     . "GREEN ITALICIZED" . $endfuncfont 
     . " headers indicate business functions."
   : '';
 # Describe "gray" authorizations
 $header_fragment1 =
      '<li><font color="#909090">Gray</font> authorizations are'
      . " expired or inactive.";
 ...
 $header_fragment = (scalar(%inactive_user))  # If there are inactive users
   ? '<li><font color="red">Red</font> Kerberos usernames are deactivated'
     . '  -- their authorizations should be deleted. '
    : '';
 # Describe the meaning of "(r)" if the user has set the "rolesflag"
 # parameter in the URL.
 $header_fragment2 = ($rolesflag eq 'Y')  # If rolesflag parm is set
    ? '<li>(r) Users flagged with "(r)" have a username/password in the Roles'
      . '  Database that allows them to maintain authorizations.'
    : '';
 # Describe the meaning of italics if the user has set the expand_auth
 # parameter in the URL to 'Y'.
 $header_fragment3 = ($expand_auth eq 'Y')  # If expand_auth parm is set
    ? "<li><i>(Italicized authorizations in parentheses)</i>"
      . " are implied by authorizations at a higher level in the tree,"
 ...
 if ($cat) {
   if (!$child_code) {
     print "This is a hierarchical display of qualifiers of type $qt"
           ...
           . " of category $cat.\n"
           . " <ul>"
           . "$header_fragment0 $header_fragment1"
           . " $header_fragment $header_fragment2"
           . " $header_fragment3"
           . "</ul>"
     ...
     print "This is a display of all paths within the $qt hierarchy"
           ...
           . " authorizations of category $cat."
           . " <ul>"
           . "$header_fragment0 $header_fragment1"
           . " $header_fragment $header_fragment2"
           . " $header_fragment3"
...
#
 ...
 if ( (!$child_code) && ((!$is_big_branch) && (!$suppress)) 
...
#
 ...
 for ($i = 0; $i < $n; $i++) {
   my $qqcode = ($itemtype[$i] eq 'Q' && $hide_qualcode eq 'Y') 
                ? 'o' 
                : $bold . $qcode[$i] . $endbold;
   $qname_string = ($itemtype[$i] eq 'Q') 
                   ? $bold . $qname[$i] . $endbold
                   : $qname[$i];
   if ($qhaschild[$i] eq 'Y' && $itemtype[$i] eq 'Q') {  # has child => URL
     if ($hide_qualcode eq 'Y') {
       $qcode_string = $bold . $qqcode . $endbold;
       $qname_string = '<A HREF="' . $url_stem
            . 'rootnode=' . $qid[$i] . '">' 
            . $bold . $qname[$i] . $endbold . '</A>';
     }
     else {
       $qcode_string = '<A HREF="' . $url_stem
            . 'rootnode=' . $qid[$i] . '">' 
            . $qqcode . '</A>';
       $qname_string = $bold . $qname[$i] . $endbold;
     }
   }
   # Special case - message about Root-level auths being suppressed
   elsif ($qname[$i] =~ /^Authorizations for root/ && $itemtype[$i] eq 'A') {
     $qname_string = '<i>' . $qname[$i] . '</i> (<A HREF="' . $url_stem
         . 'rootnode=' . $qid[$i-1] . '&showrootauths=Y';
     if ($child_code) {$qname[$i] .= "&childqc=$child_code";} # 3/7/01
     $qname_string .= '">Show them</A>)';
     $qcode_string = $bold . $qqcode . $endbold;
   }
   ...
   unless ($itemtype[$i] eq 'F' && $group_option eq '1') {
     print $qdisplay[$i] . $qcode_string . $qname_subtype . $qname_string;
   }
   if (($qt eq 'COST' || $qt eq 'FUND') && $itemtype[$i] eq 'Q' &&
     ...
     if ($l_code =~ /^FC/) {
       ...
       print ' <A HREF="' . $url_stem 
            . 'qualtype=ORG2+%28New Org. Units%29'
            . '&rootcode=' . $l_code . '&levels=2">'
            . "*</A>";
     }
   }
...
sub get_descendents {
  ...
  #
  my ($aastring, $aadf, $aacurrent, $aafunc, $aavirtreal);
  my @mqid = ();
  my @mqcode = ();
  my @mqname = ();
  my @mqhaschild = ();
  ...
  if ($cat &&  # Skip it if there is no category
      $lev != 1 && 
      ($is_big_branch || $qualid_list{$rootnode} || $expand_auth eq 'Y')
     ) {
    ...
    my $prev_func = '';
    my $prev_qcode = '';
    LISTOFAUTHS: while (($aastring, $aadf, $aacurrent, $aafunc, $aavirtreal) 
                        = $gcsr3->fetchrow_array)
    {
      ...
      $aastring 
         = &colorize_authstring($aastring, $aadf, $aacurrent, $aavirtreal);
...
sub get_qualid_list {
  ...
  my $table_or_view = 'authorization';
  $stmt6 = "select distinct a.qualifier_id"
     . " from $table_or_view a, qualifier_descendent qd"
     . " where"
...
#       $is_current = N, then strike outthe whole auth string.
#    3. If auth. is a virtual (expanded) auth, then make it gray.
#
#  Returns the adjusted $authstring.
#
###########################################################################
sub colorize_authstring {
  my ($authstring, $do_function, $is_current, $virtreal) = @_;
  my $kerbname = $authstring;
  $kerbname =~ s/\W.*//;  # Keep only the first word.
  if ($inactive_user{$kerbname}) {
    # Make the first word red and possibly italics
...
sub subtype_desc {
  ...
  for ($qualcode) {
    ...
    elsif (/^[0-9]{8}$/) {return $pfx . 'New org. unit' . $postfx;}
  }
  return $pfx . '?' . $postfx;
}
...
sub get_special_select {
  ...
  my $sort_order = ($sort_option eq 'NAME' || $hide_qualcode eq 'Y')
                   ? "3, 2"
		   : "2, 3";
  my $stmt = 
