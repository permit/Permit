...
sub main {
 ...
 if ( my $user = $g_user ) {
    ...
    ### user into mdept
    my $lda = &login_dbi_sql('mdepto');
    if(!$lda ) { 
	# print $cgi->header();
	&web_error("DB error in login to EHS database. "
	...
	#print something here for not valid user
        print $cgi->start_html();
	print "Sorry - you are not authorized to run this web report "
	    . " (username='$user').<BR>";
...
sub update_obj_link{
    ...
    for($d_ind = 0; $d_ind < $d_num; $d_ind++){
        ...
        $d_csr->bind_param(":user", $user);
        $d_csr->bind_param(":dept_id", $dlc_id);
        $d_csr->bind_param(":type", $d_type);
        $d_csr->bind_param(":code", $d_code);
        $d_csr->bind_param_inout(":message", \$message, 255);
        $error = get_db_error_msg($lda);
        if($error){return $error;}
        $d_csr->execute;
        $error = get_db_error_msg($lda);
    ...
    for($a_ind = 0; $a_ind < $a_num; $a_ind++){
        ...
        $a_csr->bind_param(":user", $user);
        $a_csr->bind_param(":dept_id", $dlc_id);
        $a_csr->bind_param(":type", $a_type);
        $a_csr->bind_param(":code", $a_code);
        $a_csr->bind_param_inout(":message", \$message, 255);
        $error = get_db_error_msg($lda);
        if($error){return $error;}
        $a_csr->execute;
        $error = get_db_error_msg($lda);
...
sub diff_link_obj{
    ...
    for($p_ind = 0; $p_ind < $p_num; $p_ind++){
        ...
        for($db_ind = 0; $db_ind < $db_num; $db_ind++){
            ...
            if($p_code and ($db_type eq $p_type) 
               ...
               and ($db_name eq $p_name)){
                $db_and_p{$db_common_key} = 1;
                break;
            }
        }
    }
    ...
    for($p_ind = 0; $p_ind < $p_num; $p_ind++){
        ...
        if(($pp_type eq $p_type) and 
           ($pp_code eq $p_code) and
           ($pp_name eq $p_name)){next;}
        $pp_type = $p_type;
        $pp_code = $p_code;
        $pp_name = $p_name;
    ...
    for($db_ind = 0; $db_ind < $db_num; $db_ind++){
        ...
        if(($pdb_type eq $db_type)
           and ($pdb_code eq $db_code)
           and ($pdb_name eq $db_name)){next;}
        $pdb_type = $db_type;
        $pdb_code = $db_code;
        $pdb_name = $db_name;
...
sub delete_dept_parent{
    ...
    $csr->bind_param(":dept_id", $dept_id);
    $csr->bind_param(":view_subtype_id", $view_subtype_id);
    $csr->bind_param(":par_dlc_id", $par_dlc_id);
    $csr->bind_param_inout(":message", \$message, 2047);
    $error = get_db_error_msg($lda);
    if($error){return $error;}
    $csr->execute;
    $error = get_db_error_msg($lda);
...
sub update_department_info{
    ...
    $csr->bind_param(":user", $user);
    $csr->bind_param(":dept_id", $dept_id);
    $csr->bind_param(":d_code", $d_code);
    $csr->bind_param(":short_name", $short_name);
    $csr->bind_param(":name", $name);
    $csr->bind_param(":dept_type_id", $dept_type_id);
    $csr->bind_param(":view_subtype_id", $view_subtype_id);
    $csr->bind_param(":parent_id", $parent_id);

    $csr->bind_param_inout(":message", $r_message, 2047);
    $error = get_db_error_msg($lda);
    $csr->execute;
    $error = get_db_error_msg($lda);
    return $error;
...
sub add_dlc{
    ...
    $csr->bind_param(":view_subtype_id", $view_subtype_id);
    $csr->bind_param(":parent_id", $parent_id);
    $csr->bind_param_inout(":dept_id", \$dept_id, 255);
    $csr->bind_param_inout(":message", \$message, 2047);
    $error = get_db_error_msg($lda);
...
sub upd_dpt_par{
    ...
    $csr->bind_param(":user", $user);
    $csr->bind_param(":dept_id", $dept_id);
    $csr->bind_param(":old_vstid", $old_vstid);
    $csr->bind_param(":new_vstid", $new_vstid);
    $csr->bind_param(":old_pid", $old_pid);
    $csr->bind_param(":new_pid", $new_pid);
    $csr->bind_param_inout(":message", \$message, 2047);
    $error = get_db_error_msg($lda);
    if($error){return $error;}
    $csr->execute;
...
#
 ...
 &print_mdept_header("Tree of DLCs", 'http');
    print $cgi->delete($param_names->{'dlc_id'});
...
sub get_all_view_types{
    ...
    my $stmt = qq{select VIEW_TYPE_CODE, VIEW_TYPE_DESC, ROOT_DEPT_ID
...
sub generate_add_page{
    ...
    print "<BODY bgcolor=\"#fafafa\">";
    &print_mdept_header("Add a DLC or Node", 'http');
    print "<HR>", "\n";
...
sub generate_edit_obj_link_page{
    ...
    print "<BODY bgcolor=\"#fafafa\">";
    &print_mdept_header("Update Object Links for a DLC or Node", 'http');
    print "<HR>\n";
...
#    print "<br><b>Current View Type</b>&nbsp;$p_view_type<br>\n";
    ...
    my $ind_obj = 0;
    my $add_obj_links = ($current_obj_links > 6) ? 2
	: (8 - $current_obj_links);
...
sub generate_edit_page{
    ...
    ############################
    ### If info is not present in params - get it form DB ###
    my $dlc_id = $p_dlc_id;
    ...
    print "<BODY bgcolor=\"#fafafa\">";
    &print_mdept_header("Update a DLC or Node", 'http');
    print "<HR>\n";
...
#
    my $result = &check_cert_source(\%ssl_info);
    if ($result ne 'OK') {
...
sub get_object_names_old {
  ...
  my $stmt = 
   "select l.object_type_code, l.object_code, q.qualifier_name
    from ${g_owner}.object_link l, qualifier\@roles q
    where l.object_type_code 
        in ('ORGU', 'ORG2', 'BAG', 'FC', 'SPGP', 'LORG', 'SIS')
    and q.qualifier_code = l.object_code
    and q.qualifier_type = decode(l.object_type_code, 'ORGU', 'ORGU', 
                                 'ORG2', 'ORG2', 'BAG', 'BAGS',
                                 'FC', 'FUND', 'SPGP', 'SPGP',
                                 'LORG', 'LORG', 'SIS', 'SISO')
...
sub get_object_names {
  ...
  my $stmt = 
   "select l.object_type_code, l.object_code, q.qualifier_name
    from ${g_owner}.object_link l, qualifier\@roles q
    where l.object_type_code 
        in ('ORGU', 'ORG2', 'BAG', 'FC', 'SPGP', 'LORG', 'SIS', 'PMIT')
    $sql_frag
    and q.qualifier_code = l.object_code
    and q.qualifier_type = decode(l.object_type_code, 'ORGU', 'ORGU', 
                                 'ORG2', 'ORG2', 'BAG', 'BAGS',
