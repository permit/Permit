...
## NAME: roles_iaps.pm
##
## DESCRIPTION: Subroutines related to IAPS qualifier feed
##  
#
...
###############################################################################
{
	...
	my($infile) = $datafile_dir . "iaps.actions";
	my($sqlfile) = $datafile_dir . "iapschange.sql";
	my($sqlfile2) = $datafile_dir . "iapsdesc.sql";
	my($qualtype) = "IAPS";
	...
	## Check number of actions to be preformed
    	...
    	$MAX_ACTIONS = &find_max_actions_value2( $db_id, $user_id, $user_pw, "MAX_IAPS");
	if (-r $infile) {
...
########################################################################
#
#  Extract IAPS data from qualifier table of Roles DB.
#
########################################################################
sub ExistingExtract
{
...
my $file1 = $data_dir . "iaps.roles";
my $file2 = $data_dir . "iaps.warehouse";
my $diff_file = $data_dir . "iaps.diffs";
my $tempactions = $data_dir . "iaps.actions.temp";
my $actionsfile = $data_dir . "iaps.actions";
my $temp1 = $data_dir . "iaps1.temp";
my $temp2 = $data_dir . "iaps2.temp";
...
system("diff $temp1 $temp2 |"            # Find differences in two files
       ...
       . " sed 's/< IAPS/<\!IAPS/' |"    # Add ! field marker
       . " sed 's/> IAPS/>\!IAPS/' |"    # Add ! field marker
