...
## NAME: roles_prin.pm
##
## DESCRIPTION: Subroutines related to qualifier feed for 'PRIN' qualifiers
##
#
...
## Created 10/28/1998, Jim Repa
###############################################################################

package roles_prin;
$VERSION = 1.0; 
$package_name = 'roles_prin';
...
###############################################################################
{
        $outfile = $datafile_dir . "prin.warehouse";
...
#
 my($lda) = login_dbi_sql($db_id, $user_id, $user_pw)
  	|| die $DBI::errstr;
 $stmt = "select q1.qualifier_code as qcode, q2.qualifier_code as qcode2, q2.qualifier_name"
          ...
          . " and q2.qualifier_code like '0H%'"
          . " order by  qcode,  qcode2";
...
#
#  Write out a made-up record for the root of the PRIN tree
#
   printf F2 "%s!%s!%s!%s\n",
           'PRIN', '0HP00_MIT', '', 
           'All Departments and Principal Investigators';
...
#
 ...
 while (($qqparcode, $qqcode, $qqname) = $csr->fetchrow_array()) {
   $qqparcode =~ s/^0HPC/0HP/;
   $qqcode =~ s/^0HPC/0HP/;
   printf F2 "%s!%s!%s!%s\n",
           'PRIN', $qqcode, $qqparcode, $qqname;
 }
...
###############################################################################
{
	...
	my($infile) = $datafile_dir . "prin.actions";
	my($sqlfile) = $datafile_dir . "princhange.sql";
	my($sqlfile2) = $datafile_dir . "prindesc.sql";
	my($qualtype) = "PRIN";
	...
	$MAX_ACTIONS = &find_max_actions_value2( $db_id, $user_id, $user_pw, "MAX_PRIN");
	if (-r $infile) {
...
########################################################################
#
#  Extract PRIN data from qualifier table of Roles DB.
#
########################################################################
sub ExistingExtract
{
...
my $file1 = $data_dir . "prin.roles";
my $file2 = $data_dir . "prin.warehouse";
my $diff_file = $data_dir . "prin.diffs";
my $tempactions = $data_dir . "prin.actions.temp";
my $actionsfile = $data_dir . "prin.actions";
my $temp1 = $data_dir . "prin1.temp";
my $temp2 = $data_dir . "prin2.temp";
...
system("diff $temp1 $temp2 |"            # Find differences in two files
       ...
       . " sed 's/< PRIN/<\!PRIN/' |"    # Add ! field marker
       . " sed 's/> PRIN/>\!PRIN/' |"    # Add ! field marker
