...
## NAME: roles_pmit.pm
##
## DESCRIPTION: Subroutines related to qualifier feed for 'PMIT' qualifiers
##
#
...
## Created 06/01/2005, Jim Repa
###############################################################################

package roles_pmit;
$VERSION = 1.0; 
$package_name = 'roles_pmit';
...
###############################################################################
{
        $outfile = $datafile_dir . "pmit.warehouse";
...
#
#  Write out a made-up record for the root of the PMIT tree.
#  (Not necessary)
#
   #printf F2 "%s!%s!%s!%s\n",
   #        'PMIT', 'PCMIT-0', '', 
...
###############################################################################
{
	...
	my($infile) = $datafile_dir . "pmit.actions";
	my($sqlfile) = $datafile_dir . "pmitchange.sql";
	my($sqlfile2) = $datafile_dir . "pmitdesc.sql";
	my($qualtype) = "PMIT";
	...
	$MAX_ACTIONS = 400;  #This should be configurable
	if (-r $infile) {
...
########################################################################
#
#  Extract PMIT data from qualifier table of Roles DB.
#
########################################################################
sub ExistingExtract
{
...
my $file1 = $data_dir . "pmit.roles";
my $file2 = $data_dir . "pmit.warehouse";
my $diff_file = $data_dir . "pmit.diffs";
my $tempactions = $data_dir . "pmit.actions.temp";
my $actionsfile = $data_dir . "pmit.actions";
my $temp1 = $data_dir . "pmit1.temp";
my $temp2 = $data_dir . "pmit2.temp";
...
system("diff $temp1 $temp2 |"            # Find differences in two files
       ...
       . " sed 's/< PMIT/<\!PMIT/' |"    # Add ! field marker
       . " sed 's/> PMIT/>\!PMIT/' |"    # Add ! field marker
