...
## 1.)  use 'budg_feed';
##	 or
##	use 'budg_feed' 1.0;  #This will specify a minimum version number
##
...
@EXPORT_OK = 
 qw(GetSequenceBUDG IncrSequenceBUDG GetBUDGFilename GenControlFile 
    GenControlFile2 RunPGPforSAPdropbox);

$TEST_MODE = 0;            ## Test Mode (1 == ON, 0 == OFF)
if ($TEST_MODE) {print "TEST_MODE is ON for budg_feed.pm\n";}

$sequence_file 
 = $ENV{'ROLES_HOMEDIR'} . '/data/budg_feed.sequence_number';
$g_pgp_cmd = "/opt/PGP/pgp";

###############################################################################
sub GetBUDGFilename	#Get filename of the BUDG extract file for SAP dropbox
###############################################################################
{
  my($date, $seqno, $filename);
  if ($TEST_MODE) {print "In $package_name:GetBUDGFilename.\n";}
      
  chomp($date = `date '+%Y%m%d%H%M%S'`);
  $seqno = &GetSequenceBUDG();
  $seqno = substr($seqno, -3, 3);  # Get last 3 digits
  $filename = "drldbbra.$seqno.$date";
  return $filename;

}

###############################################################################
sub GetSequenceBUDG	#Read in the next sequence number
###############################################################################
{
  my($seqno);
  if ($TEST_MODE) {print "In $package_name:GetSequenceBUDG.\n";}
...
###############################################################################
sub IncrSequenceBUDG # Increment and write out the next sequence number
###############################################################################
{
  my($seqno, $rc);
  if ($TEST_MODE) {print "In $package_name:GetSequenceBUDG.\n";}
...
sub RunPGPforSAPdropbox {
 ...
 #
  print "Running: '"
      . "$g_pgp_cmd -cta $input_path -z $passphrase +batchmode +force" . "'\n";
  my $rc = 
   system("$g_pgp_cmd -cta $input_path -z $passphrase +batchmode +force");
