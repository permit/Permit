...
#
 ...
 ### For now, we only support qualifier_type = 'DEPT'
...
#
 ...
 my @func_id = ();
 my %func_id2name = ();
 &get_function_list($lda, $cat, $qtype, \$n_function, \@func_id, 
                                        \%func_id2name);
...
# 
 ...
 &get_auth_list($lda, $cat, $qtype, \%qual_func_to_kerb, \%g_kerb_to_name);
...
sub get_auth_list {
  ...
  while (my @row =  $csr->fetchrow_array())
  { 
    ($qcode, $fid, $kerbname, $expand, $name) = @row;
