...
#
 ...
 $host = $ENV{'HTTP_HOST'};
 $url_stem = "http://$host/cgi-bin/rolequal1.pl?"; # URL for qualifiers
...
sub get_auths {
  ...
  #
    ...
    $csr = $lda->prepare("$stmt")
  	|| &web_error($DBI::errstr);
    $csr->execute();
    ($qqid, $ffid, $fqt, $qqtd) = $csr->fetchrow_array();
    $csr->finish() || &web_error("can't close cursor");
    if ($qqid eq '') {return;}
    if ($qual_code2) {
      @stmt = ("select q.qualifier_id, f.function_id, f.qualifier_type,"
      ...
      $csr =  $lda->prepare("$stmt")
    	|| &web_error($DBI::errstr);
      $csr->execute();
      ($qqid2, $ffid2, $fqt2, $qqtd2) = $csr->fetchrow_array() ;
      $csr->finish() || &web_error("can't close cursor");
