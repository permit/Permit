...
#  Modified 12/17/2007. Add hint in SELECT in get_auth_count_by_qualtype
#
###########################################################################
#
# Get packages
...
#
 ...
 $url_stem = "/cgi-bin/qualauth.pl?detail=2&";
...
#
 if ($action eq "first_page") {
   ...
   #print "category='$category' dept_code='$dept_code'<BR>";
...
sub tree_branch_list {
  ...
  #
  #  Get a count of authorizations by function under the DLC
  #
  my %func2auth_count = ();
  &get_auth_count_by_qualtype($lda, $category, $dept_code, \%func2auth_count);
  ...
  "select distinct f.function_category, f.qualifier_type, 
    ...
    from function f, qualifier q1, qualifier_child qc, qualifier q2,
...
$csr->execute();
  ...
  my @qualifier_code = ();
  my @qualifier_name = ();
  my ($qcategory, $qqtype, $qqcode, $qqname);
  
  while ( ($qcategory, $qqtype, $qtype_desc, $qqcode, $qqname) 
          = $csr->fetchrow_array() )
  {
  ...
  #
   ...
   for ($i=0; $i < $n; $i++) {
      ...
      my $levels_string = ($qualifier_type[$i] eq 'PCCS' 
                           || $qualifier_type[$i] eq 'COST')
                          ? ''
		          : "&levels=20";
      ...
      foreach $func_name (split($g_delim, $func_list)) {
        my $temp_count = $func2auth_count{$func_name};
...
###########################################################################
#
#  Subroutine get_auth_count_by_qualtype($lda, $category, $dept_code,
#                                        \%func2auth_count);
#
###########################################################################
sub get_auth_count_by_qualtype {
  my ($lda, $category, $dept_code, $rfunc2auth_count) = @_;

  #
  #  Open a cursor for a select statement
  #
  my $stmt = 
  "select /*+ ORDERED */ f.qualifier_type, f.function_name, 
         count(distinct a.authorization_id)
    from qualifier q1, primary_auth_descendent pad, authorization a, 
         function f
    where f.qualifier_type <> 'NULL'
    and f.function_category = '$category'
    and a.function_category = '$category'
    and q1.qualifier_type = 'DEPT'
    and q1.qualifier_code = '$dept_code'
    and pad.parent_id = q1.qualifier_id
    and pad.is_dlc = 'Y'
    and a.function_id = f.function_id
    and a.qualifier_id = pad.child_id
    group by f.qualifier_type, f.function_name";
  #print "'$stmt'<BR>";
  #my $csr = &ora_open($lda, "$stmt");
  my $csr = $lda->prepare("$stmt") or die( $DBI::errstr . "\n");
  $csr->execute();
  ...
  #  Get a list of function_names by category
  #
  while ( ($fqualtype, $fname, $acount) = &ora_fetch($csr) )
  {
    $$rfunc2auth_count{$fname} = $acount;
  }
  $csr->finish() || die "can't close cursor";
