...
#
 ...
 print "<HEAD><TITLE>$title</TITLE></HEAD>", "\n";
  ...
  #print "INFO: '$info' USER: '$k_principal' DOMAIN: '$domain'";
...
sub get_person_info {
  ...
  $stmt = "select distinct CONCAT(last_name , ', ' , first_name),"
    . " p.kerberos_name, email_addr,"
#    . " decode(status_code, 'A', '', 'inactive ')"
    . " CONCAT(CASE status_code WHEN  'A' THEN  '' ELSE  'inactive ' END "
#    . " || decode(primary_person_type,'E','employee','S','student','other'),"
    . " , CASE primary_person_type WHEN 'E' THEN 'employee' WHEN 'S' THEN 'student' ELSE 'other' END),"
    . " dept_code, q.qualifier_name, p.mit_id, a.function_category"
    . " from person p left outer join  qualifier q on p.dept_code=q.qualifier_code and  q.qualifier_type = 'ORGU'"
    . "  left outer join authorization a on a.kerberos_name = p.kerberos_name "
...
#    . " and a.kerberos_name(+) = p.kerberos_name"
    . " union select distinct CONCAT(last_name,', ' ,first_name),"
    . " p.kerberos_name, email_addr,"
#    . " decode(status_code, 'A', '', 'inactive ')"
#    . " || decode(primary_person_type,'E','employee','S','student','other'),"
    . " CONCAT(CASE status_code WHEN  'A' THEN  '' ELSE  'inactive ' END "
...
#    . " || decode(primary_person_type,'E','employee','S','student','other'),"
    . " dept_code, q.qualifier_name, p.mit_id, a.function_category"
    . " from person p left outer join  qualifier q on p.dept_code=q.qualifier_code and  q.qualifier_type = 'SISO'"
    . "  left outer join authorization a on a.kerberos_name = p.kerberos_name "
#    . " from person p, qualifier q, authorization a"
    . " where p.kerberos_name = '$lookname'"
    . " and p.primary_person_type = 'S'"
