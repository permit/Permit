...
sub get_cc_info {
  ...
  my $stmt = "select w.cost_collector_id_with_type,"
           ...
           . " DATE_FORMAT(w.cost_collector_effective_date,'%m/%d/%Y'),"
           . " DATE_FORMAT(w.cost_collector_expiration_date,'%m/%d/%Y'),"
           . " IFNULL(w.term_code, '(none)'),"
           . " w.cost_collector_expiration_date - round(NOW()),"
           . " w.supervisor_room, w.addressee, w.addressee_room,"
           . " CASE w.release_strategy WHEN '1' THEN 'Model 1' WHEN '2' THEN  'Model 2'"
           . " WHEN '3' THEN 'Model 3' WHEN '4' THEN 'Model 4' WHEN '5' THEN 'Bates Model'"
           . " WHEN 'A' THEN 'Sloan Model' ELSE 'Unknown' END"
...
sub get_department {
  ...
  my $csr = $lda->prepare("$stmt")
	|| &web_error("DB error in open. "
           ...
           . "<BR><BR>" . $DBI::errstr);
  $csr->execute();
  my ($dept_code) = $csr->fetchrow_array() ;
  $csr->finish() || &web_error("DB error. can't close cursor");
...
$csr->execute();
  ...
  $csr->execute();
  my $prev_fn = '';
  while (($aakerb, $aafn, $aaqc, $aaname, $aasort) = $csr->fetchrow_array() )
  {
