...
##############################################################################
#
# CGI script to list Authorizations (all categories) for people "within a
...
# allowing him to view other people's authorizations in all categories.
#
#
...
# Modified by Jim Repa, 6/2/2000 - Text and title changes
# Modified by Jim Repa, 11/14/2000 - Text change for Filter Option
#
##############################################################################
#
# Get packages
...
# Stem for a url for a users's authorizations
 $url_stem3 = "/cgi-bin/my-auth.cgi?category=ALL+All+Categories&FORM_LEVEL=1&";  
...
#    
 ...
 print "<HTML>", "\n";
 print "<HEAD><TITLE>Authorizations List for a DLC",
       "</TITLE></HEAD>", "\n";
 print '<BODY bgcolor="#fafafa">';
 $header = "Authorizations for people authorized on qualifiers"
      . " within $picked_qual";
...
if (!(&verify_metaauth_category($lda, $k_principal, 'ALL'))) {
  ...
  " to view other people's authorizations in all categories.";
  exit();
}
...
#
%category_list = ();
&get_categories($lda, \%category_list);
...
sub print_sap_auths {
    ...
    my (@akerbname, @afn, @aqc, @adf, @agandv, @adescend, @amoddate, @modby,
        ...
        $aacat, $csr, $n, @stmt, $prev_kerbname, $prev_cat, $kerbstring);
    ...
    #
print << 'ENDOFTEXT';
Below is a list of authorizations for users who have one or more
...
##############################################################################
#
#  Subroutine get_categories($lda, \%category_list)
#
#  Builds the %category_list hash, containing each category along with
#  its corresponding description.
#
##############################################################################
sub get_categories {
  my ($lda, $rcategory_list) = @_;
  my @stmt = ("select function_category, function_category_desc"
              . " from category");
  #my $csr = &ora_open($lda, "@stmt")
  #          || die $ora_errstr;
  my ($ccat, $cdesc);
  %$rcategory_list = ();  # Hash of categories and descriptions
   ...
   while ( ($ccat, $cdesc) = $csr->fetchrow_array() ) {
    $$rcategory_list{&strip($ccat)} = $cdesc;
   }
  $csr->finish();
  #&ora_close($csr) || die "can't close cursor";    
  $$rcategory_list{'ALL'} = 'All categories';  # Add "ALL"
}
