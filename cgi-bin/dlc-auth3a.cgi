...
sub allow_another_report {
    ...
    my $checkb3 = ($filter_opt eq '3') ? 'CHECKED' : '';
    my $checkb2 = ($checkb1 || $checkb3) ? '' : 'CHECKED';
    ...
    "2. Show all authorizations for selected people<br>",
    "<INPUT TYPE=RADIO NAME=filter VALUE=3 $checkb3>",
    "3. Show auths. for qualifiers directly related to department plus"
    . " NULL qualifiers</td></tr>",
...
sub print_options_desc {
    ...
    if ($filter_opt eq '1') {
      print " You also selected filter option 1,"
        ...
        . " To show only authorizations with NULL qualifiers or"
        . " qualifiers under $picked_qual, choose"
        . " <A HREF=\"$url_stem" . "format=$format&filter=3"
        . "&sort=$sort_opt"
        . "&qualcode=$picked_qual\">filter option 3</A>.";
    }
    elsif ($filter_opt eq '2') {
      ...
      print " You also selected filter option 3,"
        . " which finds people who have authorizations for qualifiers"
        . " under department $picked_qual, and shows their authorizations"
        . " under the departmnent plus authorizations with a NULL qualifier"
        . " To hide authorizations with NULL"
        . " qualifiers, choose"
        . " <A HREF=\"$url_stem" . "format=$format&filter=1"
        . "&sort=$sort_opt"
        . "&qualcode=$picked_qual\">filter option 1</A>."
...
sub get_select_stmt {
    ...
    my $sql_frag7 = '';
    if ($filter_opt eq '3') {
      $sql_frag7 = &get_sql_dept_list($lda, $picked_qual, $super_dept);
    }
...
sub find_super_dept {
  ...
  my @stmt = ("select /*+ ORDERED */ q2.qualifier_code"
              ...
              . "  and pad.is_dlc = 'Y')");
  #print @stmt;
my $csr = $lda->prepare("$stmt") or  &web_error("Oracle error in open. "
...
###########################################################################
#
#  Function get_sql_dept_list($lda, $dept_code, $super_dept)
#
#  Given a department code, finds a list of all children of the given
#  department code, plus the given department code.  If $super_dept is
#  not null, then add the parent department code to the list.
#
#  Then, generate a SQL fragment of the form
#     and (a.qualifier_code = 'NULL' or q3.qualifier_code in (...list...)
#  where ...list... is the quoted list of department codes from the first
#  step.
#
#  Return this SQL fragment.
#
###########################################################################
sub get_sql_dept_list {
  my ($lda, $dept_code, $super_dept) = @_;

  #
  #  Define a cursor for selecting a list of department codes.
  #
   my $stmt = "select q1.qualifier_code"
               . " from qualifier q1"
               . " where q1.qualifier_type = 'DEPT'"
               . " and q1.qualifier_code = '$dept_code'"
               . "union select q2.qualifier_code"
               . " from qualifier q1, qualifier_child qc, qualifier q2"
               . " where q1.qualifier_type = 'DEPT'"
               . " and q1.qualifier_code = '$dept_code'"
  ...
  #
  #  Execute the select and get an array of departments 
  #
   $csr->execute;
   my @dept_list = ();
   while (($another_dept) = $csr->fetchrow_array) {
     push(@dept_list, $another_dept);
   }
   $csr->finish;

  #
  #  If there is a $super_dept, add it to the list
  #
   if ($super_dept) {
     push(@dept_list, $super_dept);
   }

  #
  #  Start building the SQL fragment
  #
   my $sql_frag = " and (a.qualifier_code = 'NULL' or q3.qualifier_code";
   my $n = @dept_list;  # How many departments in list?
   if ($n == 1) {  # If 1, then ...q3.qualifier_code = '$dept_code'
     $sql_frag .= " = '$dept_list[0]')";
   }
   else {
     $sql_frag .= " in (";
     my $first = 1;
     foreach $another_dept (@dept_list) {
       if ($first) {
         $first = 0;
       }
       else {
