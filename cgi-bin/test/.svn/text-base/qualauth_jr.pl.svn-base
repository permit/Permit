...
#       08/29/01 (Add '(r)' next to username if he/she has a Roles username)
...
#
 ...
 $http_https = ($ENV{"SSL_CLIENT_DN"}) ? 'https' : 'http';
...
if ($cat) {  # Is there a category?
  ...
  $result = &check_cert_source(\%ssl_info); # Check other certificate fields
  if ($result ne 'OK') {
...
#  users have a Roles username/password), then go get a list of 
#  people who have Roles usernames.
#
 %rdb_user = ();
 if ($rolesflag eq 'Y') {
...
#
 ...
 if ($cat) {  # Headers for displays with authorizations in a category
   ...
   if (!$child_code) {
     $header = 
      "Roles Database: $list_cat Authorizations within the $qt hierarchy";
   }
   else {
     $header = 
   ...
   if (!$child_code) {
     $header = "Roles Database: Display of $qt hierarchy";
   }
   else {
...
#???                ? $bold . "<small>" . $qname[$i] . "</small>" . $endbold
 ...
 print "<A HREF=\"$main_url\"><small>Back to main Roles web interface page</small></A>";
 print "</BODY></HTML>", "\n";
 exit(0);
...
sub get_descendents {
  ...
  if ($cat &&  # Skip it if there is no category
                        ...
                        = $gcsr3->fetchrow_array)
    {
      ...
      #####
      if ($rolesflag) {  # Flag users who have Roles DB username/password
...
#  name to see if it has a Roles username/password, and if so flags it
