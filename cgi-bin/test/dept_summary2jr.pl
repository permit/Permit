...
sub get_dept_info {
  ...
  #
  my $prev_code = '';
  my ($ddparent, $ddcode, $ddname, $ddchild, $ddcname);
  while (($ddparent, $ddcode, $ddname, $ddchild, $ddcname) = &ora_fetch($csr))
  {
 ...
 #
  ...
  if ($option eq '1') {
      print "<tr>${th1}Parent<br>Dept$th2${th1}Department code$th2"
        ...
        . "${th1}Std. FC$th2${th1}PC Node$th2"
        . "${th1}Org.<br>Unit$th2"
        . "${th1}New Org. Unit$th2"
        . "${th1}LDS<br>Org.$th2"
  ...
  foreach $parent (sort keys %dept_parent) {
    ...
    foreach $child (sort @child_list) {
      ...
      if ($option eq '1') {
        print "$td1$sg$td2$td1$std_fc$td2$td1$pc_node$td2"
 ...
 #
  ...
  if ($option eq 1) {
    @type_list = ('Spending Group', 'Cust. FC', 'Std. FC',
                  'PC Node', 'Org. Unit', 'New Org. Unit', 'LDS Org.', 
                  'NIMBUS B.A.G.', 'SIS Org.')
  }
  else {
    @type_list = ('PC Node', 'Org. Unit', 
                  'NIMBUS B.A.G.', 'SIS Org.')
  }
...
sub get_std_equal_cust_fc {
  ...
  while (($cust_fc, $std_fc, $std_fc_name) = &ora_fetch($csr))
  {
