...
## perform the Roles -> PDDIFF feed.
## 
## PRECONDITIONS:
##
## 1.)  use 'pddiff_feed';
##	 or
##	use 'pddiff_feed' 1.0;  #This will specify a minimum version number
##
## 2.)	$PDORG_DATADIR environment variable must be set
##
## POSTCONDITIONS:   
##
## 1.) None.
...
##
## 8/29/2000 Jim Repa. -Created.
##
###############################################################################

package pddiff_feed;
$VERSION = 1.0;
$package_name = 'pddiff_feed';
...
@EXPORT_OK = 
 qw(GetSequencePDDIFF IncrSequencePDDIFF GetPDDIFFFilename GenControlFile);

$TEST_MODE = 0;            ## Test Mode (1 == ON, 0 == OFF)
if ($TEST_MODE) {print "TEST_MODE is ON for pddiff_feed.pm\n";}

$sequence_file 
 = $ENV{'PDORG_DATADIR'} . 'pddiff_feed.sequence_number';

###############################################################################
sub GetPDDIFFFilename #Get filename of the PDDIFF extract file for SAP dropbox
###############################################################################
{
  my($date, $seqno, $filename);
  if ($TEST_MODE) {print "In $package_name:GetPDDIFFFilename.\n";}
      
  chomp($date = `date '+%Y%m%d%H%M%S'`);
  $seqno = &GetSequencePDDIFF();
  $seqno = substr($seqno, -3, 3);  # Get last 3 digits
  $filename = "drldbpdo.$seqno.$date";
  return $filename;

}

###############################################################################
sub GetSequencePDDIFF	#Read in the next sequence number
###############################################################################
{
  my($seqno);
  if ($TEST_MODE) {print "In $package_name:GetSequencePDDIFF.\n";}
...
###############################################################################
sub IncrSequencePDDIFF # Increment and write out the next sequence number
###############################################################################
{
  my($seqno, $rc);
  if ($TEST_MODE) {print "In $package_name:GetSequencePDDIFF.\n";}
