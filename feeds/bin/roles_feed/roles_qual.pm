...
#
 my $stmt0 = "select IFNULL(is_sensitive, 'N') from qualifier_type"
 ...
 my $is_sensitive;
 ($is_sensitive) = $csr0->fetchrow_array();
 $csr0->finish();
#
...
#
 ...
 $csr->execute()
        || die $DBI::errstr;
 ($max_qual_id) = $csr->fetchrow_array();
...
#
 ...
 $i = -1;
 while ((($qqid, $qqcode, $qqlevel, $qhaschild,$qstatus) =  $csr->fetchrow_array())) {
   if (($i++)%5000 == 0) {print $i . "\n";}
   $rquid{$qqcode} = $qqid;
...
#
  ...
  #print F3 "SET AUTOCOMMIT=0;\n";	
  #print F3 "START TRANSACTION;\n";	
...
if ($action eq 'ADD') {
     ...
     if ($parentid eq '') {
       ...
       if ( ($qcode =~ /^FC\_/) || ($qcode =~ /^P\_/) ) {
         ...
         print F3 "insert into qualifier (QUALIFIER_ID, QUALIFIER_CODE,"
            ...
            . " '$qualtype','$custom_hierarchy','N','$new_level' ,NOW());\n";
     ...
     if ($auth_qcode{$qcode} == 1) {
       ...
       print F3 "update qualifier set status = 'I'"
             ...
             . " and IFNULL(status,'A') =  'T';\n"; 
       print F3 "update qualifier set status = 'I',"
             . "last_modified_date = NOW()"
             . " where qualifier_code =  '$qcode'"
             . " and qualifier_type = '$qualtype'" 
             . " and IFNULL(status,'A') = 'A';\n";     
     }
 ...
 print F3 "update qualifier set status = null, last_modified_date= NOW() where status = 'T' and "
                                                . "qualifier_type = '$qualtype';\n"; 
...
#
 ($min_id, $max_id) = $csr->fetchrow_array();
...
#
 ...
 my $rec_count = 0;
 while (($par, $chi) = $csr->fetchrow_array()) {
 ...
 $csr->finish() || die "can't close cursor 2";
...
#
 ...
 $i = 0;
 while (($qqcode, $qqid) = $csr->fetchrow_array()) {
   if (($i++)%5000 == 0) {print $i . "\n";}   
   $qid_of{$qqcode} = $qqid;
 }
 $csr->finish() || die "can't close cursor 3";
...
sub get_old_ancestor {
 ...
 #  Read in the rows
 #
 while (($par_id) = $csr->fetchrow_array()) {
   push(@old_ancestor, $par_id);
   $qid_of{$qqcode} = $qqid;
 }
 $csr->finish() || die "can't close cursor get_old_ancestor";
}
...
sub get_qual_in_auth {
 ...
 while (($qqcode) = $csr->fetchrow_array()) {
...
#    from join qualifier_child qc
#    on  q.qualifier_id =  qc.parent_id
#    where qc.child_id is null
#    and q.qualifier_type = '$qualtype'
#    and q.has_child = 'Y')";
my $statement = "update qualifier q
    set has_child = 'N',
    last_modified_date = NOW() 
    where not exists 
    (select qc.parent_id
...
#    where qualifier_id in
#    (select q.qualifier_id
#    from qualifier q left outer join qualifier_child qc
#    on q.qualifier_id = qc.parent_id
#    where qc.child_id is not null
#    and q.qualifier_type = '$qualtype'
#    and q.has_child = 'N')";
 #my $csr1 = $lda->prepare($statement)
 #       || die $DBI::errstr;
 $result = $lda->do($statement)
        || die $DBI::errstr;
 if ($result eq '0E0') {$result = 0;}
