...
## NAME: roles_ehst.pm
##
## DESCRIPTION: Subroutines related to qualifier feed for 'EHST' qualifiers
##
#
...
## Created 04/14/2006, Jim Repa
## Modified by Marina Korobko 08/11/2007
## --  use table roles_parameters to find MAX_ACTIONS allowed  
###############################################################################

package roles_ehst;
$VERSION = 1.0; 
$package_name = 'roles_ehst';
...
###############################################################################
{
        $outfile = $datafile_dir . "ehst.warehouse";
...
#
#  Open connection to EHS oracle database.  Select a record for
#  each trigger type.
#
 ...
 my($lda) = &login_dbi_sql($db_id, $user_id, $user_pw)
  	||  "$DBI::errstr . \n";
 #print "db_id='$db_id' user_id='$user_id'\n";
 my $stmt = "select 'ALL_TRIGGERS', tt.trigger_type_code || '_TRIGGERS',
              'All ' || tt.trigger_type_name || 's'
             from ehs\$user.tr_trigger_type tt
             where tt.trigger_type_code in ('R', 'S', 'G')
             order by 3";
...
#
#  Write out a made-up record for the root of the EHST tree.
#  (Not necessary)
#
   printf F2 "%s!%s!%s!%s\n",
           'EHST', 'ALL_TRIGGERS', '', 
           'All EHS Triggers for Training';

#
#  Read in the EHST records and write them out to the intermediate file.
#
 print "Reading in EHST records from EHS tables...\n";
 $i = -1;
 my ($qqparcode, $qqcode, $qqname, $node_level);
 while (($qqparcode, $qqcode, $qqname, $node_level) = $csr->fetchrow_array()) {
   printf F2 "%s!%s!%s!%s\n",
           'EHST', $qqcode, $qqparcode, $qqname;
 }
 $csr->finish();
...
#
 my $stmt2 = "select distinct cto.trigger_type_code || '_TRIGGERS', 
              cto.trigger_type_code || cto.trigger_object_code,
              cto.trigger_object_name
    from ehs\$user.tr_cert_trigger_object cto, 
         ehs\$user.who_needs_cert_rule r, 
         ehs\$user.tr_trigger_type tt
    where r.trigger_type_code = cto.trigger_type_code
    and r.trigger_object_code = cto.trigger_object_code
    and cto.trigger_type_code in ('R', 'S', 'G')
    and tt.trigger_type_code = cto.trigger_type_code
    order by 1, 2";
...
#                        print "db_id=$db_id user_id=$user_id\n";            
	...
	$destination_lda->disconnect() || die "can't log off Oracle";
...
###############################################################################
{
	...
	my($infile) = $datafile_dir . "ehst.actions";
	my($sqlfile) = $datafile_dir . "ehstchange.sql";
	my($sqlfile2) = $datafile_dir . "ehstdesc.sql";
	my($qualtype) = "EHST";
	...
	## Check number of actions to be preformed
        $MAX_ACTIONS = &find_max_actions_value2($db_id,$user_id,$user_pw, "MAX_EHST");
	if (-r $infile) {
...
########################################################################
#
#  Extract EHST data from qualifier table of Roles DB.
#
########################################################################
sub ExistingExtract
{
...
#  Open first cursor
#
 my $stmt = "select q2.qualifier_code, q.qualifier_id, q.qualifier_code," 
...
##############################################################################
#
#  Find the differences in two files of EHST objects.
...
my $file1 = $data_dir . "ehst.roles";
my $file2 = $data_dir . "ehst.warehouse";
my $diff_file = $data_dir . "ehst.diffs";
my $tempactions = $data_dir . "ehst.actions.temp";
my $actionsfile = $data_dir . "ehst.actions";
my $temp1 = $data_dir . "ehst1.temp";
my $temp2 = $data_dir . "ehst2.temp";
...
system("diff $temp1 $temp2 |"            # Find differences in two files
       ...
       . " sed 's/< EHST/<\!EHST/' |"    # Add ! field marker
       . " sed 's/> EHST/>\!EHST/' |"    # Add ! field marker
